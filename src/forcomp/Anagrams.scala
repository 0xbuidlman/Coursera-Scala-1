...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ((w.toLowerCase() toList) groupBy ((element: Char) => element) map { case (k,v) => k -> (v.length) } toList) sortBy (_._1)
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s match {
    case List() => List()
    case y :: ys => ((sentenceOccurrences(ys) ::: wordOccurrences(y)) groupBy ( kv => kv._1) map { case (k,v) => k -> (v map ( kv => kv._2) sum ) }  toList) sortBy (_._1)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (for {
      (x_Cha, x_Cnt) <- x
    } yield (x_Cha, x_Cnt - (y.toMap.getOrElse(x_Cha,0)) )
    ...
    def sentenceAnagrams0(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case _ => {
        (for {
          ...
          sentenceAnagrams0(subtract(occurrences,os)) map (word :: _) 
        }) flatten
      }
    }
